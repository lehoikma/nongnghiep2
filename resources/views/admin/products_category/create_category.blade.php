...
@section('content')
    ...
    <div class="col-md-12">
        ...
        <form method="post" action="{{route('save_products_category')}}">
            {{csrf_field()}}
            <div class="col-md-8">
                <div class="form-group">
                    <input type="text" name="name_category_news" class="form-control" placeholder="Nhập tên danh mục ...">
                </div>
            </div>
            <div class="col-md-4">
                ...
                @if(!empty($categoriesNews))
                    @foreach($categoriesNews as $value)
                        <tr>
                            ...
                            <td>
                                <a href="{{route('form_edit_products_category', $value['id'])}}">
                                    <button class="btn btn-warning btn-sm" data-id="{{$value['id']}}"><i class="fa fa-trash"></i> Sửa</button>
                                </a>
                                <a href="{{route('delete_products_category', $value['id'])}}">
