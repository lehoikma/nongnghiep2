                ...
                @foreach($news as $new)
                    <div class="col-xs-12 item-news" style="margin-bottom: 30px">
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 news-content">
                            <a style="font-family: SFD-Bold;font-size: 20px;line-height: 26px;color: #333;text-decoration: none;display: block;margin-bottom: 10px;font-weight: bold;margin-top: -5px;"
                               href="{{route('detail_news', $new['id'])}}">
                                {{$new['title']}}
                            </a>
                            <span class="time-posted">Ngày: {{date_format(date_create($new['created_at']), 'Y-m-d')}}</span>
                            <p>{{substr(strip_tags($new['content']), 0, 220)}} ...</p>
                        </div>
                    </div>
                @endforeach
            @endif
